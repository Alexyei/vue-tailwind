<template>
    ...
    <label
        @click="setSync"
        :class="{'bg-transparent bg-none z-2 shadow-picked':isChecked,
        'bg-dark-light shadow-checkbox bg-light text-dark-blue shadow-npicked':!isChecked,
        'hover:shadow-picked':value !== ''}"
        class="for-checkbox-tools break-all font-Abel text-xxs sm:text-xs font-bold transition-all duration-300 ease-linear text-white uppercase cursor-pointer overflow-hidden rounded text-center relative block px-0.5 sm:px-1.25 pt-1 sm:py-3 w-1/6 max-w-16 sm:max-w-full sm:w-24 h-21 sm:h-25.5 tracking-one my-0 mx-1 sm:mx-1.25 mb-2.5 " for="tool-1">
...
export default {
  ...
  created() {
    ...
    // if (this.value && this.modelValue.has(this.value))
    //   this.picked = true
  ...
  methods: {
    ...
    setSync(){
      ...
      if (!this.isChecked){
        ...
        this.modelValue.add(this.value)}
      else
        //this.modelValue.delete(this.cardKey)
        this.modelValue.delete(this.value)
