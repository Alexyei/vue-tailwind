<template>
  ...
  <div class="lg:container px-8 w-full flex justify-between items-center">
    ...
    <div class="flex flex-col justify-between items-center relative w-7.5 h-7.5 cursor-pointer z-10 md:invisible md:opacity-0"  @click="isActive = !isActive">
      ...
      <span class="block w-7.5 h-0.75 my-0.75 mx-auto transition duration-300 ease-in-out bg-text-gray" :class="{'transform translate-y-2.5 rotate-45':isActive}" ></span>
      <span class="block w-7.5 h-0.75 my-0.75 mx-auto transition duration-300 ease-in-out bg-text-gray" :class="{'opacity-0':isActive}"></span>
