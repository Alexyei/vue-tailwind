<template>
  <teleport to="body">
    <div class="fixed top-0 bottom-0 right-0 left-0 flex items-center justify-center bg-popup-gray" :class="{'opacity-100 z-9000':isActive, 'opacity-0 -z-1':!isActive}">
      <div class="mx-4 popup center absolute shadow-popup bg-white rounded-lg p-6 w-112.5 box-border z-2 text-center transform scale-50 opacity-0 transition-all duration-500 ease-in-out" :class="{'opacity-100 top-1/2 scale-100 -translate-y-1/2':isActive, '-top-2full ':!isActive}">
...
// },
  ...
  methods: {
    show(message, status = "success",  header=""){
      this.status = status;
